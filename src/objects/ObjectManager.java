...
public class ObjectManager {
    ...
    private void updateProjectiles(int[][] lvlData, Player player) {
        for(Projectile p : projectiles)
            ...
            if (p.getHitbox().intersects(player.getHitbox())){
                ...
                p.setActive(false);
            } else if (IsProjectTileHittingLevel(p, lvlData)){
                p.setActive(false);
            }
    ...
    private void drawPotions(Graphics g, int xLvlOffset) {
        for (Potion p: potions)
            if(p.isActive()) {
                int type = 0;
                if(p.objType == RED_POTION)
                    type = 1;
                g.drawImage(potionImgs[type][p.getAniIndex()], (int)(p.getHitbox().x - p.getxDrawOffset() - xLvlOffset), (int)(p.getHitbox().y - p.getyDrawOffset()), POTION_WIDTH,
                        POTION_HEIGHT, null);
            }
    }
